...
void esp_event(const char* data)
{
    printf("\r\nDebug recv : %s\r\n", data);
    if (strstr(data, "RAIN@ON"))
    {
        LCD_Puts(0, 0, "Rainy Day");
        LCD_Puts(0, 1, "Take Umbrella");
    }
    else if (strstr(data, "RAIN@OFF"))
    {
        LCD_Puts(0, 0, "No Rain");
        LCD_Puts(0, 1, "Have a nice day!");
    }
}
